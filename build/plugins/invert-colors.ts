...
export const invertColors: esbuild.Plugin = {
  ...
  setup(build) {
    build.onLoad({ filter: /\.css$/ }, async (args) => {
      ...
      // Invert grayscale colors and accommodate alpha channels in the CSS content
      const invertedContents = updatedContents.replace(/#([0-9A-Fa-f]{3,6})([0-9A-Fa-f]{2})?\b/g, (match, rgb, alpha) => {
        ...
        // Check if the color is grayscale (R, G, and B components are equal)
        if (r === g && g === b) {
          // Invert RGB values
          const invertedColorValue = (255 - r).toString(16).padStart(2, "0");
          // Return the inverted grayscale color with alpha channel if present
          return `#${invertedColorValue}${invertedColorValue}${invertedColorValue}${alpha || ""}`;
        }

        // If the color is not grayscale, return it as is, including the alpha channel if present
        return `#${color}${alpha || ""}`;
      });
