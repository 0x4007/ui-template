...
async function fetchIssueFromPullRequest(pullRequest: GitHubPullRequest, issues: GitHubIssue[]): Promise<GitHubIssue | null> {
  ...
  // Match the issue reference in the PR body using RegExp.exec()
  const issueUrlRegex = /Resolves (https:\/\/github\.com\/([^/]+)\/([^/]+)\/issues\/(\d+))/;
  const issueNumberRegex = /Resolves #(\d+)/;
  const issueMarkdownLinkRegex = /Resolves \[\s*#(\d+)\s*\]/;

  let apiUrl: string;
  const issueUrlMatch = issueUrlRegex.exec(pullRequest.body);
  const issueNumberMatch = issueNumberRegex.exec(pullRequest.body);
  const issueMarkdownLinkMatch = issueMarkdownLinkRegex.exec(pullRequest.body);
  ...
  if (issueUrlMatch) {
    ...
    } else if (issueMarkdownLinkMatch) {
      issueNumber = issueMarkdownLinkMatch[1];
    }
    if (!issueNumber) return null;

    const pullRequestUrlRegex = /repos\/([^/]+)\/([^/]+)\/pulls\/\d+/;
    const pullRequestUrlMatch = pullRequestUrlRegex.exec(pullRequest.url);
...
function countBackLinks(aggregated: GitHubAggregated, allPullRequests: GitHubPullRequest[], allIssues: GitHubIssue[]): number {
  ...
  function countMatches(body: string | null, repo: string, owner: string): number {
    ...
    // Use RegExp.exec() for each pattern
    if (prFullUrlRegex) {
      prFullUrlRegex.lastIndex = 0; // Reset lastIndex since we're using global flag
      if (prFullUrlRegex.exec(body)) count++;
    }

    if (issueFullUrlRegex) {
      issueFullUrlRegex.lastIndex = 0;
      if (issueFullUrlRegex.exec(body)) count++;
    }

    if (issueShortRefRegex && repo === repoName && owner === ownerName) {
      issueShortRefRegex.lastIndex = 0;
      if (issueShortRefRegex.exec(body)) count++;
    }
...
export async function fetchAllNotifications(): Promise<GitHubAggregated[] | null> {
  ...
  // filter notifications with priority label
  const filteredNotifications = allNotifications.filter((aggregated) => {
    ...
    const priorityLabelRegex = /^(Priority): /;
    const hasPriorityLabel = aggregated.issue.labels.some((label) => {
      if (typeof label === "string" || !label.name) return false;
      return priorityLabelRegex.exec(label.name) !== null;
    });
